...
```{r design, echo=T, fig.width=3, fig.height=3, fig.align="center", cache=TRUE}
tt <- tibble(
...
sigPeps_total <- list(sigPeps, sigPepsPerChip) %>%
    unlist(recursive = F) %>%
    unlist(recursive = F)
```
...
krsa(sigPeps$meanLFC$`0.2`,
    return_count = T, seed = 123, itr = 2000,
