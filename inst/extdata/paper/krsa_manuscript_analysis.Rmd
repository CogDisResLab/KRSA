...
krsa_heatmap(data_modeled$normalized, sigPeps$meanLFC[["0.2"]], groups = comparisons$Comp1, scale = "row", cluster_col = F)
...
krsa_violin_plot_grouped(data_modeled$grouped, sigPeps$meanLFC[["0.2"]], grp_comp = comparisons,
                   avg_line =  T
                   )
...
# generates a curve plot represenating the linear model fit for the selected peptide (top peptides)
...
# load in chip coverage and kinase-substrate files OR uplaod your own files
...
krsa_ball_model(kinases_hits, AvgZTable, 15, 2.5, 4.8)
...
```{r reading}
raw <- readxl::read_xlsx("data/Export Tis100 brain data extended.xlsx", sheet = 4, col_names = F,
                         na = c("NA", "")
                         )
...
#tidydata_krsa <- krsa_read("data/ad_data_full.txt")
...
tidydata2 %>% filter(Peptide %in% sd_peps_2, Peptide != "ART_025_CXGLRRWSLGGLRRWSL") %>% 
...
future_map(ad_sigPeps_total, krsa) -> ad_mutiple_krsa_outputs
...
erica <- read_csv(file = "~/Desktop/Master_Classes/mcculm/Erica/KRSA_Paper/bioinformatics_manuscript/KRSA_results.csv")

erica %>% filter(ZScore >= 1.5) %>% pull(Kinase) -> erc_kin_5
fin %>% filter(abs(Z) >= 1.5) %>% pull(Kinase) -> ad_kin_5

erica %>% filter(ZScore >= 1.25) %>% pull(Kinase) -> erc_kin_1
fin %>% filter(abs(Z) >= 1.25) %>% pull(Kinase) -> ad_kin_1


x2 <- list(`DLPFC`=erc_kin_5 , `HPC`=ad_kin_5
           )
pdf("../AD/venn_diagram_15.pdf",useDingbats = F, width = 9, height = 8)
RAM::group.venn(x2,
                cat.cex = 1,
                lab.cex = 1,
                cat.pos = c(0,0),
...
x2 <- list(`DLPFC`=erc_kin_1 , `HPC`=ad_kin_1
           )
pdf("../AD/venn_diagram_125.pdf",useDingbats = F, width = 9, height = 8)
RAM::group.venn(x2,
                cat.cex = 1,
                lab.cex = 1,
                cat.pos = c(0,0),
