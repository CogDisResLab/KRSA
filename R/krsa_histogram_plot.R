...
krsa_histogram_plot <- function(data,data2,kinases) {
  data2 %>% rename(Kinase = Kin) %>%
    ...
    geom_rect(data=filter(data, Kinase %in% kinases),aes(xmin=SamplingAvg+(2*SD), xmax=SamplingAvg-(2*SD), ymin=0, ymax=Inf),
              fill="gray", alpha=0.5
    ) +
    geom_vline(data=filter(data, Kinase %in% kinases), aes(xintercept = Observed), color = "red", size = 1,show.legend = F) +
